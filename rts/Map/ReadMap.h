...
public:
	...
	/// BlockMaps
	const BlockMap<float>& GetMIPHeightMapBlockMap(unsigned int mip) const { return mipHeightMaps[mip]; }
	const BlockMap<float3>& GetCenterNormals2DBlockMap()  const { return centerNormals2D; }
	const BlockMap<uint8_t>& GetTypeMapBlockMap() const { return typeMap; }
	      BlockMap<uint8_t>& GetTypeMapBlockMap()       { return typeMap; }
...
protected:
	...
	/**
	 * array of BlockMaps of heightmaps in different resolutions,
	 * mipPointerHeightMaps[0  ] is full resolution (centerHeightMap),
	 * mipPointerHeightMaps[n+1] is half resolution of mipPointerHeightMaps[n]
	 */
	std::vector<BlockMap<float>> mipHeightMaps;

	BlockMap<uint8_t> typeMap;
	BlockMap<float3> centerNormals2D;
